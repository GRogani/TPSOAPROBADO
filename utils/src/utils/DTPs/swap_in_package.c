#include "swap_in_package.h"

int send_swap_in_package(int socket, uint32_t pid) {
    t_package* package = create_swap_in_package(pid);
...
t_package *create_swap_in_package(uint32_t pid){
    ...
    return create_package(UNSUSPEND_PROCESS, buffer);
}

uint32_t read_swap_in_package(t_package* package) { // Lectura para Memoria
